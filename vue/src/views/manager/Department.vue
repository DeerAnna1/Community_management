    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="description" label="社团介绍" show-overflow-tooltip>
          <template v-slot="scope">
            <el-button type="success" @click="viewEditor(scope.row.description)">查看详细介绍</el-button>
          </template>
        </el-table-column>
      ...
      <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
        ...
        <el-form-item prop="userId" label="社长姓名">
          <el-select v-model="form.userId" placeholder="请选择社长" style="width:100%">
            <el-option v-for="item in leaderData" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userId: [
          {required: true, message: '请选择社长', trigger: 'blur'},
        ]
      },
      ids: [],
      leaderData:[],
  ...
  methods: {
    ...
    loadLeaders(){
      this.$request.get('/user/getAllLeaders').then(res =>{
        if (res.code === '200'){
